...
# 初始化数据存储
accuracy_scores = {prompt_type: [] for prompt_type in prompt_types}
...
for prompt_type in prompt_types:
    for batch_size in batch_sizes:
        ...
        if not os.path.exists(result_dir):
            ...
            accuracy_scores[prompt_type].append(None)  # 如果目录不存在，添加None作为占位符
            continue
        ...
        if not os.path.exists(eval_file):
            ...
            accuracy_scores[prompt_type].append(None)
            continue
        ...
        try:
            with open(eval_file, 'r') as f:
                ...
                if accuracy is not None:
                    accuracy_scores[prompt_type].append(accuracy)
                else:
...
for i, prompt_type in enumerate(prompt_types):
    # 过滤掉None值
    valid_indices = [j for j, score in enumerate(accuracy_scores[prompt_type]) if score is not None]
    valid_batch_sizes = [batch_sizes[j] for j in valid_indices]
    valid_scores = [accuracy_scores[prompt_type][j] for j in valid_indices]
...
# 添加图表标题和标签
plt.title('Comparison of Accuracy for different prompt\ntypes and batch sizes', 
...
# 保存图表
output_path = output_dir / 'accuracy_line_chart.png'
